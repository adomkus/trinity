...
input int             InpPeakSearchPeriod   = 5;       // Periodas pikų paieškai (barais į kairę/dešinę)
input double          InpPeakProminenceATR  = 1.5;     // Piko "iškilumo" koeficientas (ATR vienetais)

//--- Istorijos ribojimo nustatymas
input group           "History Settings"
input int             InpLookbackBars       = 1000;    // Kiek žvakių atgal skaičiuoti ir piešti
                ...
                const int &spread[])
{
    ...
    PeakPoint peaks[];
    // Apribojame skaičiavimą iki pasirinkto istorijos gylio (saugikliai ir ribojimai)
...
//+------------------------------------------------------------------+
//| Patobulinta pikų ir dugnų paieškos funkcija                      |
//+------------------------------------------------------------------+
int FindAdvancedPeaks(PeakPoint &peaks[], const int rates_total, const double &high[], const double &low[], const datetime &time[], const int start_index, const int end_index)
{
    ...
    // --- 1 etapas: Randame visus potencialius pivot taškus
    ...
    // --- 2 etapas: Filtruojame pagal "iškilumą" (prominence)
